...
// - verificationId (string): backend verification ID
// - userData (object): all collected registration data
// - onSuccess: callback when verified and countdown completes
// - onBack: () => void
const CustomerCodeConfirmation = ({ email, verificationId, userData, onSuccess, onBack }) => {
  ...
  const handleVerifyCode = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await apiService.verifyCode({
        verificationId,
        code: codeString,
        userData,
      });
      ...
      if (ok) {
        ...
        // Store user session like logged-in users
        await AsyncStorage.setItem('user_session', JSON.stringify(result.data.user));
        // Clear registration data
        await AsyncStorage.multiRemove(['customer_registration_step1', 'customer_registration_step2']);
  ...
  // Redirect after countdown completes to avoid triggering parent state during child render
  useEffect(() => {
    if (verifiedSuccess && redirectCountdown === 0) {
      onSuccess?.();
    }
  }, [verifiedSuccess, redirectCountdown, onSuccess]);
