...
const CodeConfirmation = ({ verificationId, phoneNumber, onVerificationSuccess, onBack }) => {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Persist rider details immediately so Profile/Main can read full data
        try {
          await AsyncStorage.setItem('rider:user', JSON.stringify(result.user));
        } catch (e) {
          // Non-blocking
        }

        // Show success UI and start 5s countdown before redirect
  ...
  const handleResendCode = async () => {
    ...
    try {
      // Find working URL dynamically
      const baseURL = await findWorkingUrl();
      console.log('🌐 Using working URL for resend:', baseURL);
