...
const CreateAccount1 = ({ onNext, onBack }) => {
  ...
  const handleNext = async () => {
    ...
    // Save data locally
    try {
      await AsyncStorage.setItem('rider_registration_step1', JSON.stringify(userData));
      console.log('✅ Step 1 data saved locally:', userData);
    } catch (error) {
        ...
        <View style={styles.inputContainer}>
          ...
          <Animated.Text style={[
            styles.floatingLabel,
            {
              ...
              paddingHorizontal: passwordLabelAnimation.interpolate({
                inputRange: [0, 1],
                outputRange: [0, 4],
              }),
              backgroundColor: passwordLabelAnimation.interpolate({
                inputRange: [0, 1],
                outputRange: ['transparent', '#fff'],
              }),
            }
