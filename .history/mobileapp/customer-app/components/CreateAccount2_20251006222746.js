...
const CreateAccount2 = ({ regData, addressData, onSuccess }) => {
  ...
  useEffect(() => {
    const keyboardDidShowListener = Keyboard.addListener('keyboardDidShow', () => {
      Animated.timing(formContainerTranslateY, {
        toValue: -220, // Lift the form container up
        duration: 250,
  ...
  useEffect(() => {
    (async () => {
      try {
        const saved = await AsyncStorage.getItem('customer_registration_step2');
        if (saved) {
          const parsed = JSON.parse(saved);
          setLocalAddr(prev => ({ ...prev, ...parsed }));
        }
      } catch {}
    })();
  }, []);

  // Persist as user edits fields or map
  useEffect(() => {
    (async () => { try { await AsyncStorage.setItem('customer_registration_step2', JSON.stringify(localAddr)); } catch {} })();
  }, [localAddr]);
  ...
  const submitRegistration = async () => {
    ...
    try {
      ...
      const userData = {
        ...(step1 ? JSON.parse(step1) : regData),
        ...(step2 ? JSON.parse(step2) : localAddr),
        role: 'customer',
        // normalize phone key for backend expectations
        phone_number: (step2 ? JSON.parse(step2)?.phoneNumber : localAddr?.phoneNumber) || (step1 ? JSON.parse(step1)?.phoneNumber : regData?.phoneNumber) || '',
      };

      // Request verification code via email
      const send = await apiService.sendVerificationCode({ 
        email: userData.email, 
        phoneNumber: userData.phone_number || null, 
        role: 'customer' 
      });
      if (!send.success) {
        throw new Error(send.error || 'Failed to send verification code');
      }

      // Hand off to verification screen
      onSuccess({ 
        pendingVerification: true, 
        verificationId: send.data.verification_id, 
        userData 
      });
    } catch (e) {
    ...
    <View style={styles.createContainer}>
      ...
      <Animated.View style={[
        ...
        <View style={styles.welcomeContainer}>
          ...
          <Text style={styles.subtitle}>Don’t keep your rider guessing, drop your pin!</Text>
        </View>
