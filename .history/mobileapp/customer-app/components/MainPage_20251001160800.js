...
const MainPage = ({ user, onLogout }) => {
  ...
  // Animated values for search field transition (using translateY instead of margin/padding)
  ...
  const handleScroll = (event) => {
    ...
    if (shouldBeScrolled !== isScrolled) {
      ...
      // When scrolled: move down 30px to compensate for removed negative margin
      // When not scrolled: stay at 0
      Animated.timing(searchTranslateY, {
        toValue: shouldBeScrolled ? 30 : 0,
        duration: 300,
...
const styles = StyleSheet.create({
  ...
  searchContainer: {
    backgroundColor: '#FFFFFF',
    paddingBottom: 10,
    paddingTop: 60, // Static top padding for sticky state
  },
  searchWrapper: {
    paddingHorizontal: 20,
    marginTop: -30, // Keep the initial overlap
  },
