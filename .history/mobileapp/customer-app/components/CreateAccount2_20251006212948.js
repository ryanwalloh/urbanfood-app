...
const CreateAccount2 = ({ regData, addressData, onSuccess }) => {
  ...
  // Persist step2 data as it changes
  useEffect(() => {
    (async () => {
      try {
        await AsyncStorage.setItem('customer_registration_step2', JSON.stringify(localAddr));
      } catch {}
    })();
  }, [localAddr]);
  ...
  const submitRegistration = async () => {
    ...
    try {
      ...
      const send = await apiService.sendVerificationCode({ email: userData.email, phoneNumber: null, role: 'customer' });
      if (!send.success) {
        throw new Error(send.error || 'Failed to send verification code');
      }

      // Hand off to verification screen
      onSuccess({ pendingVerification: true, verificationId: send.data.verification_id, userData });
    } catch (e) {
