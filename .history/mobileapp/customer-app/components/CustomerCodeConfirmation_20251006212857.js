...
import { findWorkingUrl, apiService } from '../services/api';

const { width } = Dimensions.get('window');

const CustomerCodeConfirmation = ({ verificationId, userData, onVerificationSuccess, onBack }) => {
  ...
  const handleVerifyCode = async () => {
    ...
    setIsLoading(true);
    try {
      // Merge latest persisted data to be safe
      const step1 = await AsyncStorage.getItem('customer_registration_step1');
      const step2 = await AsyncStorage.getItem('customer_registration_step2');
      const mergedUser = { ...(step1 ? JSON.parse(step1) : {}), ...(step2 ? JSON.parse(step2) : {}), ...userData, role: 'customer' };

      const res = await apiService.verifyCode({ verificationId, code: codeString, userData: mergedUser });
      if (res.success && res.data?.success) {
        setHasVerified(true);
        await AsyncStorage.removeItem('customer_registration_step1');
        await AsyncStorage.removeItem('customer_registration_step2');
        try { await AsyncStorage.setItem('customer:user', JSON.stringify(res.data.user)); } catch {}
        ...
        resetCode();
        Alert.alert('Error', res.data?.error || res.error || 'Verification failed');
      }
    } catch (e) {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.successContainer}>
          <Image source={require('../assets/verified.png')} style={styles.successIcon} resizeMode="contain" />
  ...
  return (
    <View style={styles.container}>
      <Image source={require('../assets/createaccount2.png')} style={styles.backgroundImage} resizeMode="cover" />
      <View style={styles.darkOverlay} />
      <View style={styles.logoContainer}>
        <Image source={require('../assets/sotihorizontal2.png')} style={styles.logo} resizeMode="contain" />
      </View>
      <Animated.View style={[styles.formContainer, { transform: [{ translateY: formContainerTranslateY }] }]}>
        <View style={styles.welcomeContainer}>
          ...
          <Text style={styles.subtitleText}>We sent a 5-digit code to{"\n"}<Text style={styles.phoneNumber}>{userData?.email}</Text></Text>
        </View>
        <Animated.View style={[styles.codeInputContainer, { transform: [{ translateX: shakeAnimation }] }]}>
          {code.map((digit, index) => (
            <TextInput
              ...
              style={[styles.otpBox, currentFocus === index && styles.otpBoxFocused, digit !== '' && styles.otpBoxFilled, showError && styles.otpBoxError]}
              value={digit}
        ...
        <TouchableOpacity style={[styles.verifyButton, (isLoading || code.length !== 5) && styles.verifyButtonDisabled]} onPress={handleVerifyCode} disabled={isLoading || code.length !== 5}>
          <Text style={styles.verifyButtonText}>{isLoading ? 'Verifying...' : 'Verify Code'}</Text>
        </TouchableOpacity>
        <View style={styles.backContainer}>
          <TouchableOpacity onPress={onBack}><Text style={styles.backLink}>← Back to Registration</Text></TouchableOpacity>
        </View>
      </Animated.View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  welcomeText: { fontSize: 24, fontFamily: 'Nexa-Heavy', color: '#333333', marginBottom: 10, textAlign: 'center' },
  subtitleText: { fontSize: 14, fontFamily: 'Nexa-ExtraLight', color: '#666666', textAlign: 'center', opacity: 0.9, lineHeight: 20 },
  phoneNumber: { fontFamily: 'Nexa-Heavy', color: '#F43332' },
  ...
  otpBox: { width: 50, height: 60, borderWidth: 1, borderColor: '#DDD', borderRadius: 12, textAlign: 'center', fontSize: 24, fontFamily: 'Nexa-Heavy', color: '#333', backgroundColor: '#FFFFFF' },
  ...
  timerText: { fontSize: 14, fontFamily: 'Nexa-ExtraLight', color: '#666666', textAlign: 'center' },
  resendButton: { paddingVertical: 8, paddingHorizontal: 16 },
  resendButtonText: { fontSize: 14, fontFamily: 'Nexa-Heavy', color: '#F43332', textAlign: 'center' },
  ...
  verifyButtonText: { color: '#FFFFFF', fontSize: 18, fontFamily: 'Nexa-Heavy' },
  backContainer: { alignItems: 'center' },
  backLink: { fontSize: 16, fontFamily: 'Nexa-ExtraLight', color: '#F43332', textDecorationLine: 'underline' },
});
