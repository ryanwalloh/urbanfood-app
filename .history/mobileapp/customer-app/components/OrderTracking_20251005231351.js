...
// Animated Line Component that follows actual route
const AnimatedLine = ({ riderLocation, customerLocation, animatedValue }) => {
  ...
  useEffect(() => {
    ...
    const listener = animatedValue.addListener(({ value }) => {
      ...
      // Get the coordinates up to the current progress point
      const animatedCoords = routeData.coordinates.slice(0, animatedPointCount + 1);
      
      setRouteData(prev => ({
        ...prev,
        animatedCoordinates: animatedCoords
      }));
    });
  ...
  }, [routeData, animatedValue]);

  if (!routeData) return null;

  return (
    <>
      {/* Semi-transparent background route */}
      <Polyline
      ...
      {routeData.animatedCoordinates.length > 0 && (
        <Polyline
          coordinates={routeData.animatedCoordinates}
          strokeColor="#F43332"
          strokeWidth={4}
        />
      )}
