...
// Animated Polyline Component for moving line effect
const AnimatedPolyline = ({ coordinates, animatedValue, strokeColor, strokeWidth }) => {
  const [animatedCoords, setAnimatedCoords] = useState([]);

  useEffect(() => {
    const listener = animatedValue.addListener(({ value }) => {
      const totalPoints = coordinates.length;
      const animatedPointCount = Math.floor(totalPoints * value);
      setAnimatedCoords(coordinates.slice(0, animatedPointCount + 1));
    });

    return () => {
      animatedValue.removeListener(listener);
    };
  }, [coordinates, animatedValue]);
...
const OrderTracking = ({ order: initialOrder, restaurant, cartItems, user, onBack, address }) => {
  ...
  // Animation for moving line effect
  ...
  const fetchRiderLocation = async (orderData) => {
    try {
      ...
      if (response.success && response.location) {
        ...
        if (customerLocation) {
          calculateRoute(newRiderLocation, customerLocation);
        }
  ...
  // Calculate route between rider and customer
  const calculateRoute = async (riderLoc, customerLoc) => {
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${riderLoc.latitude},${riderLoc.longitude}&destination=${customerLoc.latitude},${customerLoc.longitude}&key=AIzaSyCCuDLJMhB-23kQiXYpXwi-yYGvKz7OgSQ`
      );
      ...
      if (data.status === 'OK' && data.routes.length > 0) {
        ...
        console.log('🗺️ Route calculated with', decodedCoordinates.length, 'points');
  ...
  // Start animated moving line effect
  const startAnimatedLine = () => {
    const animate = () => {
      Animated.sequence([
        Animated.timing(animatedLineProgress, {
          toValue: 1,
          duration: 3000, // 3 seconds to complete the line
        ...
        if (isTrackingRider && routeCoordinates.length > 0) {
          animate();
        }
      });
    };
  ...
  // Calculate animated line coordinates based on progress
  const getAnimatedLineCoordinates = (progress) => {
    if (routeCoordinates.length === 0) return [];
    
    const totalPoints = routeCoordinates.length;
    const animatedPointCount = Math.floor(totalPoints * progress);
    
    return routeCoordinates.slice(0, animatedPointCount + 1);
  };
