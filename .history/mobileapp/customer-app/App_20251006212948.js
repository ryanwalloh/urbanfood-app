...
export default function App() {
  ...
  const [pendingVerification, setPendingVerification] = useState(null); // {verificationId, userData}
  ...
  const handleRegistrationSuccess = (payload) => {
    if (payload?.pendingVerification) {
      setPendingVerification({ verificationId: payload.verificationId, userData: payload.userData });
      setCurrentPage('customerVerify');
      return;
    }
    setUser(payload);
    ...
    <View style={styles.container}>
      ...
      {currentPage === 'customerVerify' && (
        <CustomerCodeConfirmation
          ...
          onVerificationSuccess={(u) => { setUser(u); setCurrentPage('onboarding1'); }}
