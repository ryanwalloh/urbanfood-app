...
export const apiService = {
  ...
  // Send verification code (email primary, SMS fallback). Accepts minimal user info
  sendVerificationCode: async ({ email, phoneNumber, role = 'customer' }) => {
    try {
      ...
      const client = axios.create({ baseURL: workingUrl, timeout: 10000, headers: { 'Content-Type': 'application/json' } });
      const payload = {
        email,
        phone_number: phoneNumber,
        role,
      };
      const res = await client.post('/users/send-verification-code/', payload);
  ...
  verifyCode: async ({ verificationId, code, userData }) => {
    try {
      let workingUrl = cachedWorkingUrl;
      if (!workingUrl) {
        workingUrl = await testMultipleUrls();
      ...
      const client = axios.create({ baseURL: workingUrl, timeout: 10000, headers: { 'Content-Type': 'application/json' } });
      const res = await client.post('/users/verify-sms-code/', {
