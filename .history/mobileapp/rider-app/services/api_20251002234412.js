...
export const apiService = {
  ...
  connectToOrderUpdates: (onMessage, onError) => {
    try {
      console.log('📡 Connecting to real-time order updates...');
      
      // We'll use the same base URL finding logic
      findWorkingUrl().then(baseURL => {
        const url = baseURL + '/orders/sse-stream/';
        ...
        eventSource.onmessage = (event) => {
          try {
            ...
            if (data.type === 'order_count_update' || data.type === 'order_count') {
              onMessage(data);
            }
          } catch (e) {
