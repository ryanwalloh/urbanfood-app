...
export const apiService = {
  ...
  connectToOrderUpdates: (onUpdate, onError) => {
    try {
      ...
      // Find working URL and convert to WebSocket URL
      findWorkingUrl().then(baseURL => {
        ...
        ws.onclose = (event) => {
          ...
          setTimeout(() => {
            ...
            apiService.connectToOrderUpdates(onUpdate, onError);
          }, 5000);
        };
        
        // Return the WebSocket for cleanup
        return ws;
      }).catch(error => {
        console.log('❌ Failed to setup WebSocket:', error);
        if (onError) onError(error);
      });
