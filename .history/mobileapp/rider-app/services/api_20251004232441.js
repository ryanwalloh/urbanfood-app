...
export const apiService = {
  ...
  // Request presigned S3 PUT URL for proof of delivery
  getPresignedUploadUrl: async (orderId, fileName, fileType) => {
    try {
      console.log('✅ Requesting presigned URL for:', orderId, fileName);
      const baseURL = await findWorkingUrl();
      const url = baseURL + '/rider/get-presigned-upload-url/';

      const body = `order_id=${encodeURIComponent(orderId)}&file_name=${encodeURIComponent(fileName)}&file_type=${encodeURIComponent(fileType || 'image/jpeg')}`;
      ...
      if (!res.ok) {
        const errorText = await res.text();
        console.log('❌ Presigned URL response error text:', errorText);
        return { success: false, error: `HTTP ${res.status}: ${errorText}` };
      }

      const data = await res.json().catch((jsonError) => {
        console.log('❌ Presigned URL JSON parsing error:', jsonError);
      ...
      console.log('❌ Presigned URL request error:', error);
