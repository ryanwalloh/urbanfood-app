...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  const handleOrderPickedUp = async () => {
    ...
    try {
      console.log('📦 Updating order status to OTW for order:', orderDetails.order_id);
      
      const result = await apiService.updateOrderStatus(orderDetails.order_id, 'otw');
      
      if (result.success) {
        Alert.alert(
          'Success', 
          'Order status updated to "On the Way"! You can now proceed to deliver the order.',
          [{ text: 'OK' }]
        );
        console.log('✅ Order status updated successfully:', result.message);
        
        // Refresh order details to get updated customer information
  ...
  // Recalculate route when both rider and customer locations are available
  useEffect(() => {
    if (riderLocation && orderDetails?.customer_street) {
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      setRegion(adjustedRegion);
        ...
        // Calculate route when both rider and customer locations are available
        if (customerLocation && orderDetails) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.callingCard}>
          ...
          <View style={styles.callingCardInfo}>
            ...
            <Text style={styles.callingCardSubtitle}>{orderDetails.customer_phone}</Text>
          </View>
        ...
        <TouchableOpacity 
          style={[styles.pickedUpButton, isUpdatingStatus && styles.pickedUpButtonDisabled]}
