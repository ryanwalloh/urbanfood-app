...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  // Define drag limits
  const EXPANDED_HEIGHT = height * 0.6; // 60% of screen height (current state)
  const COLLAPSED_HEIGHT = height * 0.3; // 30% of screen height (collapsed state)
  const DRAG_LIMIT = EXPANDED_HEIGHT - COLLAPSED_HEIGHT; // 30% difference
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        // Constrain the movement to our limits
        const constrainedValue = Math.max(-DRAG_LIMIT, Math.min(0, gestureState.dy));
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        const shouldExpand = dy < -DRAG_LIMIT / 2 || vy < -500;
        const shouldCollapse = dy > DRAG_LIMIT / 2 || vy > 500;
        ...
        if (shouldExpand && !isExpanded) {
          ...
          Animated.spring(translateY, {
            toValue: isExpanded ? -DRAG_LIMIT : 0,
            useNativeDriver: true,
            tension: 100,
            friction: 8,
          }).start();
