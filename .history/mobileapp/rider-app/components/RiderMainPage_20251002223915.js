...
const RiderMainPage = ({ onLogout }) => {
    ...
    const loadData = async () => {
      try {
        ...
        if (stored) {
          ...
          // Start periodic sync
          scheduleSync(5000); // First sync in 5 seconds
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState) => {
      ...
      if (nextAppState === 'background' || nextAppState === 'inactive') {
        ...
        setRiderOnline();
        // Sync immediately when returning to app
  ...
  const getLocationAndWeather = async () => {
    try {
      ...
      if (weatherData.status === 'fulfilled' && weatherData.value.success) {
        ...
        console.log('✅ Real-time weather loaded successfully');
      } else {
        console.log('❌ Weather fetch failed:', weatherData.reason || weatherData.value?.error);
        // Don't set weather data if API fails - this will hide the weather container
      }
    ...
    try {
      ...
      if (response.success && response.is_available !== undefined) {
        ...
        if (frontendStatus !== backendStatus) {
          console.log('⚠️ Status mismatch detected! Syncing frontend to backend...');
          
          // Update frontend to match backend (source of truth)
          setIsOnline(backendStatus);
          
          // Update overlay animation
          Animated.timing(overlayOpacity, {
            toValue: backendStatus ? 0 : 1,
            duration: 300,
            useNativeDriver: true,
          }).start();
          
          console.log('✅ Frontend synced to backend status:', backendStatus ? 'Online' : 'Offline');
        } else {
  ...
  const scheduleSync = (delay = 10000) => {
    if (syncTimeoutRef.current) {
      clearTimeout(syncTimeoutRef.current);
    }
    ...
    syncTimeoutRef.current = setTimeout(() => {
      syncWithBackend();
      scheduleSync(30000); // Next sync in 30 seconds
    }, delay);
  };

  // Function to set rider online automatically (for login)
  const setRiderOnline = async () => {
    try {
      console.log('🔄 Setting rider online automatically...');
      console.log('📊 Current isOnline state:', isOnline);
      
      // Always set online when this function is called (for app return/login)
      // 1. OPTIMISTIC UI UPDATE - Set online immediately
      setIsOnline(true);
      
      // 2. Update overlay animation immediately
      Animated.timing(overlayOpacity, {
      ...
      // 3. Call backend in background (non-blocking)
      try {
        const response = await apiService.toggleStatus();
        
        if (response.success === false) {
          console.log('❌ Backend failed to set online, but UI already updated');
          // Could rollback here if needed, but for auto-online it's usually fine to stay optimistic
        } else {
          console.log('✅ Rider set to online:', response.new_status);
        }
      } catch (apiError) {
        console.log('❌ Backend error setting online:', apiError.message);
        // For auto-online, we typically stay optimistic even on backend failure
      }
    } catch (error) {
      console.log('❌ Unexpected error setting online:', error);
    }
  };

  // Function to set rider offline automatically (for app backgrounding)
  const setRiderOffline = async () => {
    try {
      console.log('🔄 Setting rider offline automatically...');
      
      // Only set offline if currently online
      if (isOnline) {
        // 1. OPTIMISTIC UI UPDATE - Set offline immediately
        setIsOnline(false);
        ...
        try {
          ...
          if (response.success === false) {
            console.log('❌ Backend failed to set offline, but UI already updated');
            // For auto-offline, we typically stay optimistic even on backend failure
          } else {
            console.log('✅ Rider set to offline:', response.new_status);
          }
        } catch (apiError) {
          console.log('❌ Backend error setting offline:', apiError.message);
          // For auto-offline, we typically stay optimistic even on backend failure
        }
      }
    } catch (error) {
      console.log('❌ Unexpected error setting offline:', error);
    }
  };
  ...
  const toggleOnlineStatus = async () => {
    try {
      ...
      // 1. OPTIMISTIC UI UPDATE - Update immediately for instant feedback
      setIsOnline(newStatus);
      
      // 2. Animate toggle button press (immediate)
      Animated.sequence([
      ...
      // 3. Animate overlay immediately (no waiting for backend)
      Animated.timing(overlayOpacity, {
      ...
      // 4. Call backend API in background (non-blocking)
      try {
        const response = await apiService.toggleStatus();
        ...
        // 5. ROLLBACK on failure - revert to previous state
        console.log('❌ Backend error, rolling back:', apiError.message);
        ...
        // Revert animation
        Animated.timing(overlayOpacity, {
          toValue: previousStatus ? 0 : 1,
          duration: 300,
          useNativeDriver: true,
        }).start();
        
        // Show error only after rollback
        Alert.alert(
          'Error',
