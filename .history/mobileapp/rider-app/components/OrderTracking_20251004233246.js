...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      ...
      const fileName = `proof-of-delivery/${orderDetails.order_id}/${timestamp}_${proofOfDeliveryImage.fileName}`;
      ...
      const presign = await apiService.getPresignedUploadUrl(
        ...
        `${timestamp}_${proofOfDeliveryImage.fileName}`,
        proofOfDeliveryImage.mimeType || 'image/jpeg'
      );
      ...
      const { url: presignedUrl, file_url: s3Url, content_type } = presign;

      // 2) Upload the file directly to S3 using PUT
      // Use uploadAsync with PUT and set headers to match presigned params
      // Read file as binary
      const fileBinary = await FileSystem.readAsStringAsync(proofOfDeliveryImage.uri, {
        encoding: FileSystem.EncodingType.Base64,
      });
      const binaryBody = Buffer.from(fileBinary, 'base64');

      const putRes = await fetch(presignedUrl, {
        method: 'PUT',
        headers: {
          'Content-Type': content_type || (proofOfDeliveryImage.mimeType || 'image/jpeg'),
          'x-amz-acl': 'public-read',
        },
        body: binaryBody,
      });

      console.log('📤 PUT upload status:', putRes.status);
      if (!putRes.ok) {
        const errText = await putRes.text().catch(() => '');
        throw new Error(`Upload failed with status: ${putRes.status} ${errText}`);
      }

      // 3) Update order with S3 URL and set status to delivered
  ...
  useEffect(() => {
    if (riderLocation && orderDetails) {
      if (orderDetails.status === 'otw' && orderDetails.customer_street) {
        ...
        calculateRouteFromAddress(riderLocation, orderDetails.customer_street + ', ' + orderDetails.customer_barangay);
      } else if (orderDetails.restaurant_street) {
        // Route to restaurant when order is not picked up yet
        calculateRouteFromAddress(riderLocation, orderDetails.restaurant_street + ', ' + orderDetails.restaurant_barangay);
      }
    }
    ...
    try {
      ...
      setRegion(adjustedRegion);
        ...
        if (orderDetails) {
          if (orderDetails.status === 'otw' && orderDetails.customer_street) {
            // Route to customer when order is picked up (OTW)
            calculateRouteFromAddress(riderCoords, orderDetails.customer_street + ', ' + orderDetails.customer_barangay);
          } else if (orderDetails.restaurant_street) {
            // Route to restaurant when order is not picked up yet
            calculateRouteFromAddress(riderCoords, orderDetails.restaurant_street + ', ' + orderDetails.restaurant_barangay);
          }
        }
    } catch (error) {
      ...
      if (response.success) {
        ...
        if (riderLocation) {
          if (response.status === 'otw' && response.customer_street) {
            // Route to customer when order is picked up (OTW)
            calculateRouteFromAddress(riderLocation, response.customer_street + ', ' + response.customer_barangay);
          } else if (response.restaurant_street) {
            // Route to restaurant when order is not picked up yet
            calculateRouteFromAddress(riderLocation, response.restaurant_street + ', ' + response.restaurant_barangay);
          }
        }
      } else {
