...
const RiderMainPage = ({ onLogout }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // 2. Set rider offline in backend (non-blocking)
      try {
        console.log('📤 Setting rider offline in backend...');
        await apiService.toggleStatus();
        console.log('✅ Rider set offline in backend');
      } catch (error) {
        console.log('⚠️ Failed to set rider offline in backend:', error.message);
