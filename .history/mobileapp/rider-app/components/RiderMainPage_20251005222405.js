...
const RiderMainPage = ({ onLogout }) => {
    ...
    const loadData = async () => {
      try {
        ...
        if (stored) {
          ...
          // Load recent delivered transactions
  ...
  const loadRecentTransactions = async () => {
    try {
      const res = await apiService.getRecentTransactions();
      if (res.success) {
    ...
    try {
      ...
      if (response.success && response.is_available !== undefined) {
        ...
        if (frontendStatus !== backendStatus) {
          ...
          apiService.toggleStatus(frontendStatus).then(response => {
  ...
  const setRiderOnline = async () => {
    try {
      ...
      if (!isOnline) {
        ...
        // 3. Update backend (non-blocking) - send target status
        apiService.toggleStatus(true).then(response => {
  ...
  const setRiderOffline = async () => {
    try {
      ...
      if (isOnline) {
        ...
        // 3. Update backend (non-blocking) - send target status
        apiService.toggleStatus(false).then(response => {
  ...
  const toggleOnlineStatus = async () => {
    try {
      ...
      // 3. Update backend (non-blocking) - send target status
      apiService.toggleStatus(newStatus).then(response => {
  ...
  // Extract Google Plus Code from a text string (e.g., "7QF6+9Q")
  const extractPlusCode = (text) => {
    if (!text || typeof text !== 'string') return null;
    const match = text.toUpperCase().match(/\b[0-9A-Z]{4,8}\+[0-9A-Z]{2,3}\b/);
  ...
  if (currentScreen === 'orders') {
    console.log('🔄 Rendering Orders component from main page...');
    return (
      <Orders 
    ...
    <View style={styles.mainContainer}>
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.ordersContainer}>
          ...
          {/* Offline Overlay - Only show when rider is offline */}
          {!isOnline && (
          ...
          <View style={styles.transactionsContainer}>
            ...
            {(recentTransactions.length === 0) ? (
              <View style={{ padding: 16 }}>
                <Text style={[styles.transactionTimestamp, { textAlign: 'center' }]}>No recent deliveries yet</Text>
              </View>
            ) : recentTransactions.map((assignment, index) => (
                ...
                <View style={styles.transactionItem}>
                  ...
                  <View style={styles.transactionContent}>
                    <View style={styles.transactionHeader}>
                      ...
                      <Text style={styles.assignmentAmount}>+ ₱{Number(assignment.rider_fee || 0).toFixed(2)}</Text>
                    </View>
                    <Text style={styles.customerName}>{assignment.customer}</Text>
                    <Text style={styles.assignmentAddress}>
                      {assignment.barangay 
                        ? `${(assignment.plus_code || extractPlusCode(assignment.street || assignment.address)) ? `${assignment.plus_code || extractPlusCode(assignment.street || assignment.address)}, ` : ''}${assignment.barangay}, Iligan City`
                        : `${assignment.address || ''}${(assignment.plus_code || extractPlusCode(assignment.street || assignment.address)) ? `  ·  ${assignment.plus_code || extractPlusCode(assignment.street || assignment.address)}` : ''}`
                      }
                    </Text>
