...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  // PanResponder for smooth drag gestures
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (evt, gestureState) => {
        // Only respond to vertical gestures with minimal threshold for responsiveness
      ...
      onPanResponderMove: (evt, gestureState) => {
        // Constrain movement within bounds: 0 (initial) to MAX_DOWNWARD_DRAG (collapsed)
        ...
        setIsCollapsed(constrainedValue > MAX_DOWNWARD_DRAG / 2);
      },
      onPanResponderRelease: (evt, gestureState) => {
        ...
        setIsCollapsed(finalValue > MAX_DOWNWARD_DRAG / 2);
        
        // No snap animations - content stays where user releases it
        // This creates the most natural and smooth experience
      },
    })
  ).current;
