...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  useEffect(() => {
    if (riderLocation && restaurantLocation) {
      calculateRoute(riderLocation, restaurantLocation);
    }
  ...
  const calculateRoute = async (start, end) => {
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${start.latitude},${start.longitude}&destination=${end.latitude},${end.longitude}&key=AIzaSyCCuDLJMhB-23kQiXYpXwi-yYGvKz7OgSQ`
      );
