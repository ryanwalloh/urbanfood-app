...
const RiderMainPage = ({ onLogout }) => {
  ...
  const connectToOrderUpdates = () => {
    try {
      console.log('📡 Setting up WebSocket connection for order updates...');
      
      const ws = apiService.connectToOrderUpdates(
        (data) => {
          // Handle incoming order count updates
          if (data.count !== undefined) {
            setOrdersCount(data.count);
            console.log('📦 Order count update via WebSocket:', data.count);
          }
        },
        (error) => {
      ...
      console.log('❌ Failed to setup WebSocket connection:', error);
    }
  };
