...
const Orders = ({ onBack }) => {
  ...
  // Add debugging when component renders
  useEffect(() => {
    console.log('📊 Orders component state update:', {
      loading,
      ordersCount: orders.length,
      orders: orders.map(o => ({ id: o.order_id, status: o.status }))
    });
  }, [loading, orders]);
  ...
  const handleAcceptOrder = async (orderId) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        console.log('✅ Order accepted successfully, navigating to tracking');
