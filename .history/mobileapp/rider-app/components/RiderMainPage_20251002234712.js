...
const RiderMainPage = ({ onLogout }) => {
    ...
    const loadData = async () => {
      try {
        ...
        if (stored) {
          ...
          // Start order updates polling
          startOrderUpdatesPolling();
        }
      } catch (error) {
  ...
  const startOrderUpdatesPolling = () => {
    try {
      console.log('📡 Setting up order updates polling...');
      
      const polling = apiService.startOrderUpdatesPolling(
        (data) => {
          // Handle incoming order count updates
          if (data.count !== undefined) {
            setOrdersCount(data.count);
            console.log('📦 Order count update via polling:', data.count);
          }
        },
        (error) => {
          console.log('❌ Order polling error:', error);
          // Continue polling even on error - it will retry next interval
        },
      ...
      console.log('❌ Failed to setup order polling:', error);
    }
  };
