...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  // Snap points - using positive values for translateY
  const SNAP_POINTS = {
    COLLAPSED: 0, // Show only 200px of content (at bottom)
    EXPANDED: 200, // Show most of the content
    FULL: 400, // Show all content but keep some visible for dragging
  };
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        const newMapHeight = 350 - (progress * 150); // Reduce map height by up to 150px
        const newContentHeight = (height - 350) + (progress * 150); // Increase content height
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        const targetMapHeight = 350 - (progress * 150);
        const targetContentHeight = (height - 350) + (progress * 150);
