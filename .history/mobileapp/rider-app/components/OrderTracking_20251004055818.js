...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  const mapHeight = useRef(new Animated.Value(350)).current;
  const contentHeight = useRef(new Animated.Value(height - 350)).current;
  
  // Snap points - translateY values for content container
  const SNAP_POINTS = {
    COLLAPSED: 0, // Content at bottom, only drag handle visible
    EXPANDED: 150, // Content partially expanded
    FULL: 300, // Content fully expanded
  };
  ...
  useEffect(() => {
    translateY.setValue(SNAP_POINTS.COLLAPSED);
  }, []);
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        const newTranslateY = translateY._offset + gestureState.dy;
        const clampedTranslateY = Math.max(SNAP_POINTS.COLLAPSED, Math.min(SNAP_POINTS.FULL, newTranslateY));
        translateY.setValue(clampedTranslateY - translateY._offset);
        
        // Update map and content heights based on drag
        const progress = (clampedTranslateY - SNAP_POINTS.COLLAPSED) / (SNAP_POINTS.FULL - SNAP_POINTS.COLLAPSED);
        const newMapHeight = 350 + (progress * 100); // Expand map height by up to 100px
        const newContentHeight = height - 350 - (progress * 100); // Reduce content height
        
        mapHeight.setValue(newMapHeight);
        contentHeight.setValue(newContentHeight);
      },
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (velocity > 0.5) {
          // Fast downward swipe - collapse content
          if (currentSnapPoint === 'FULL') {
            targetSnapPoint = 'EXPANDED';
          } else if (currentSnapPoint === 'EXPANDED') {
            targetSnapPoint = 'COLLAPSED';
          } else {
            targetSnapPoint = 'COLLAPSED';
          }
        } else if (velocity < -0.5) {
          // Fast upward swipe - expand content
        ...
        const progress = (SNAP_POINTS[targetSnapPoint] - SNAP_POINTS.COLLAPSED) / (SNAP_POINTS.FULL - SNAP_POINTS.COLLAPSED);
        const targetMapHeight = 350 + (progress * 100); // Expand map height by up to 100px
        const targetContentHeight = height - 350 - (progress * 100); // Reduce content height
