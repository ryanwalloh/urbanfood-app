  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        // Only allow downward dragging (positive dy values)
        // Constrain movement: 0 (initial) to MAX_DOWNWARD_DRAG (collapsed)
        const constrainedValue = Math.max(0, Math.min(MAX_DOWNWARD_DRAG, gestureState.dy));
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        // Determine if we should snap to collapsed or return to initial
        const shouldCollapse = dy > MAX_DOWNWARD_DRAG / 2 || vy > 500;
        const shouldReturnToInitial = dy < MAX_DOWNWARD_DRAG / 2 || vy < -500;
        
        if (shouldCollapse && !isCollapsed) {
          // Snap to collapsed (down)
          setIsCollapsed(true);
          Animated.spring(translateY, {
            toValue: MAX_DOWNWARD_DRAG,
            useNativeDriver: true,
            tension: 100,
            friction: 8,
          }).start();
        } else if (shouldReturnToInitial && isCollapsed) {
          // Return to initial state (up)
          ...
          Animated.spring(translateY, {
            toValue: isCollapsed ? MAX_DOWNWARD_DRAG : 0,
            useNativeDriver: true,
            tension: 100,
            friction: 8,
          }).start();
