...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  const handleUploadPhoto = () => {
    setShowImagePreviewModal(false);
    // TODO: Implement AWS S3 upload here
    Alert.alert('Upload', 'Upload functionality will be implemented next!');
  };
  ...
  const renderOrderDetails = () => {
    ...
    return (
      <View style={styles.content}>
        ...
        {/* Proof of Delivery File Info */}
        {orderDetails?.status === 'arrived' && proofOfDeliveryImage && (
          <View style={styles.proofOfDeliveryInfo}>
            <Text style={styles.proofOfDeliveryLabel}>Selected file:</Text>
            <Text style={styles.proofOfDeliveryUrl} numberOfLines={2}>
              {proofOfDeliveryImage.fileName || proofOfDeliveryImage.uri}
            </Text>
            <Text style={styles.proofOfDeliverySize}>
              {proofOfDeliveryImage.fileSize ? `Size: ${Math.round(proofOfDeliveryImage.fileSize / 1024)}KB` : ''}
            </Text>
          </View>
        )}
        ...
        {orderDetails?.status === 'arrived' && (
          <View style={{ backgroundColor: '#FFE6E6', padding: 10, margin: 10, borderRadius: 5 }}>
            <Text style={{ fontSize: 10, color: '#666' }}>
              DEBUG: Status: {orderDetails?.status}, Image: {proofOfDeliveryImage ? 'YES' : 'NO'}
            </Text>
            {proofOfDeliveryImage && (
              <Text style={{ fontSize: 10, color: '#666' }}>
                File: {proofOfDeliveryImage.fileName}
              </Text>
            )}
          </View>
        )}
