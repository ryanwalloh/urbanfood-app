...
const CodeConfirmation = ({ verificationId, phoneNumber, onVerificationSuccess, onBack }) => {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.254.111:8000/users/verify-sms-code/', {
        method: 'POST',
        headers: {
  ...
  const handleResendCode = async () => {
    ...
    setIsResending(true);
    try {
      const response = await fetch('http://192.168.254.111:8000/users/resend-verification-code/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleCodeChange = (text) => {
    ...
    // Auto-submit when 5 digits are entered (with small delay to ensure state is updated)
    if (cleanText.length === 5) {
      setTimeout(() => {
        // Double-check that we still have exactly 5 digits
        if (cleanText.length === 5 && !isLoading) {
          handleVerifyCode();
        }
      }, 100);
    }
