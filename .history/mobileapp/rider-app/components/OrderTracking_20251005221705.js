...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  // Animation value for tracking dot
  const trackingDotOpacity = useRef(new Animated.Value(1)).current;
  
  // Start tracking dot animation
  const startTrackingDotAnimation = () => {
    const fadeInOut = () => {
      Animated.sequence([
        Animated.timing(trackingDotOpacity, {
          toValue: 0.3,
          duration: 500,
        ...
        // Repeat the animation if tracking is still active
  ...
  // Stop tracking dot animation
  const stopTrackingDotAnimation = () => {
    trackingDotOpacity.stopAnimation();
    trackingDotOpacity.setValue(1);
  };
  
  // Proof of delivery state

  // --- Map interaction controls -------------------------------------------
  const RIDER_TOP_OFFSET_FACTOR = 0.3; // keep rider near top by this fraction of latDelta
  // Separate, adjustable offset for initial/reload placement
  const INITIAL_TOP_OFFSET_FACTOR = 0.45; // you can tweak this value independently
  ...
  const computeRiderTopRegion = (riderCoords, baseRegion) => {
    const latDelta = (baseRegion && baseRegion.latitudeDelta) ? baseRegion.latitudeDelta : 0.005;
    const lngDelta = (baseRegion && baseRegion.longitudeDelta) ? baseRegion.longitudeDelta : 0.005;
    return {
      latitude: riderCoords.latitude + (latDelta * -RIDER_TOP_OFFSET_FACTOR),
  ...
  const computeInitialRiderTopRegion = (riderCoords, baseRegion) => {
    const latDelta = (baseRegion && baseRegion.latitudeDelta) ? baseRegion.latitudeDelta : 0.005;
    const lngDelta = (baseRegion && baseRegion.longitudeDelta) ? baseRegion.longitudeDelta : 0.005;
    return {
      latitude: riderCoords.latitude + (latDelta * -INITIAL_TOP_OFFSET_FACTOR),
      longitude: riderCoords.longitude,
      latitudeDelta: latDelta,
      longitudeDelta: lngDelta,
    };
  ...
  const [region, setRegion] = useState({
    latitude: 8.2275 + (0.005 * RIDER_TOP_OFFSET_FACTOR), // Use shared rider-top offset factor
    longitude: 124.2458,
  ...
  useEffect(() => {
    ...
    // Start real-time location tracking when component mounts
    startRealTimeTracking();
    
    // Cleanup tracking when component unmounts
    return () => {
      locationTracker.stopTracking();
      stopTrackingDotAnimation();
    };
  }, [orderId]);
  ...
  useEffect(() => {
    ...
    if ((orderDetails.status === 'otw' || orderDetails.status === 'arrived') && orderDetails.customer_street) {
      // Prefer precise coords if we have them
      if (customerLocation) {
        calculateRouteToCoords(riderLocation, customerLocation);
        return;
      }
      // Use normalized full customer address (street + barangay + country)
      const customerAddress = buildCustomerAddress(orderDetails);
      console.log('🗺️ Calculating route to customer address:', customerAddress);
  ...
  const adjustMapRegionToFitRoute = () => {
    ...
    const latPadding = rawLatSpan * 0.1;
    const lngPadding = rawLngSpan * 0.1;

    // Clamp zoom so short routes don't zoom too far out
    // Compute approximate distance between rider and last route point
    const last = routeCoordinates[routeCoordinates.length - 1];
    const toRad = (d) => (d * Math.PI) / 180;
    const R = 6371; // km
    const dLat = toRad(last.latitude - riderLocation.latitude);
    const dLon = toRad(last.longitude - riderLocation.longitude);
    const a = Math.sin(dLat/2)**2 + Math.cos(toRad(riderLocation.latitude)) * Math.cos(toRad(last.latitude)) * Math.sin(dLon/2)**2;
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
    const distanceKm = R * c;

    // Define min/max deltas based on distance thresholds
    let minDelta = 0.003; // tighter for very short routes
    let maxDelta = 0.05;  // cap for moderate routes
    if (distanceKm > 5) { maxDelta = 0.2; }
    if (distanceKm > 15) { maxDelta = 0.5; }

    // Calculate deltas to fit the route then clamp
    const computedLatDelta = rawLatSpan + latPadding;
    const computedLngDelta = rawLngSpan + lngPadding;
    const latDelta = Math.min(Math.max(computedLatDelta, minDelta), maxDelta);
    const lngDelta = Math.min(Math.max(computedLngDelta, minDelta), maxDelta);
    ...
    const centerLat = riderLocation.latitude + (latDelta * -RIDER_TOP_OFFSET_FACTOR);
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      // Update rider location in backend (backup in case it wasn't captured during order acceptance)
      console.log('📍 Updating rider location in backend (backup)');
      const locationResponse = await apiService.updateRiderLocation(latitude, longitude);
      if (locationResponse.success) {
        console.log('✅ Rider location updated successfully in backend');
      } else {
        console.log('⚠️ Failed to update rider location in backend:', locationResponse.error);
      }
      
      // Initial placement with adjustable factor; apply only once
      const adjustedRegion = computeInitialRiderTopRegion(riderCoords, region);
      ...
      if (!didInitialPlacementRef.current) {
        didInitialPlacementRef.current = true;
        setRegionManually(adjustedRegion);
      }
        ...
        if (orderDetails) {
          if ((orderDetails.status === 'otw' || orderDetails.status === 'arrived') && orderDetails.customer_street) {
            if (customerLocation) {
              ...
              const customerAddress = buildCustomerAddress(orderDetails);
              console.log('🗺️ Calculating route to customer address (location update):', customerAddress);
  ...
  const startRealTimeTracking = () => {
    ...
    // Start tracking dot animation
    startTrackingDotAnimation();
    
    // Configure tracking for delivery use case
    locationTracker.updateConfig({
      pollingInterval: 25000, // 25 seconds for delivery tracking
      movementThreshold: 0.0001, // ~10 meters movement threshold
      accuracyThreshold: 100 // Accept locations within 100m accuracy
    });

    // Start tracking with callbacks
    locationTracker.startTracking(
      orderId,
      handleLocationUpdate, // Called when significant movement detected
      handleLocationError   // Called on location errors
    );
  };

  const handleLocationUpdate = async (location, metadata) => {
    try {
      console.log('📍 Real-time location update:', location, metadata);
      
      // Update local rider location state
      setRiderLocation(location);
      
      // Update backend database
      const response = await apiService.updateRiderLocation(location.latitude, location.longitude);
      
      if (response.success) {
        console.log('✅ Real-time location updated in database:', {
          orderId: metadata.orderId,
          accuracy: metadata.accuracy + 'm',
          timestamp: metadata.timestamp
        });
      } else {
        console.log('⚠️ Failed to update real-time location:', response.error);
      }
      
    } catch (error) {
      console.log('❌ Error handling location update:', error);
    }
  };

  const handleLocationError = (error) => {
    console.log('❌ Real-time location tracking error:', error);
    // Don't show error to user unless it's critical
    // The tracking will continue to retry automatically
  };
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Calculate route based on order status
        if (riderLocation) {
          if ((response.status === 'otw' || response.status === 'arrived') && response.customer_street) {
            // Route to customer when order is picked up (OTW)
            const normalizedCustomerAddress = buildCustomerAddress(response);
            console.log('🗺️ Calculating route to customer address (fetch order):', normalizedCustomerAddress);
            calculateRouteFromAddress(riderLocation, normalizedCustomerAddress);
  ...
  const buildCustomerAddress = (orderData) => {
    if (!orderData) return '';
    const street = orderData.customer_street || '';
    const barangay = orderData.customer_barangay || '';
    // Prefer full street + barangay when available; always append country
    const parts = [];
    if (street) parts.push(street);
    if (barangay) parts.push(barangay);
    if (parts.length === 0) return 'Philippines';
    parts.push('Philippines');
    return parts.join(', ');
  };

  // Format address for floating destination container: "+code, City, barangay"
  const formatFloatingDestinationAddress = (orderData) => {
    if (!orderData) return '';
    
    let street = '';
    let barangay = '';
    
    if (orderData.status === 'otw' || orderData.status === 'arrived') {
      street = orderData.customer_street || '';
      barangay = orderData.customer_barangay || '';
    } else {
      street = orderData.restaurant_street || '';
      barangay = orderData.restaurant_barangay || '';
    }

    // If no street, just return barangay
    if (!street) {
      return barangay;
    }

    // Detect plus code format: "66HX+MX Iligan City, Lanao del Norte"
    const plusCodeMatch = street.match(/^([^\s]+\+[^\s]+)\s+(.+)$/);
    if (plusCodeMatch) {
      const plusCode = plusCodeMatch[1]; // "66HX+MX"
      const locality = plusCodeMatch[2]; // "Iligan City, Lanao del Norte"
      
      // Extract city from locality (remove province)
      // Split by comma and take the first part (city)
      const city = locality.split(',')[0].trim(); // "Iligan City"
      
      // Return formatted address: "+code, City, barangay"
      return `${plusCode}, ${city}${barangay ? `, ${barangay}` : ''}`;
    }

    // If no plus code, return original format
    return barangay ? `${street}, ${barangay}` : street;
  };
      ...
      <View style={styles.content}>
        ...
        <TouchableOpacity 
          style={[
            ...
            orderDetails?.status === 'otw' && styles.otwArrivedButton
          ]}
    ...
    >
      <View style={styles.successModalOverlay}>
        <View style={styles.successModalContent}>
          ...
          {/* Title */}
          <Text style={styles.successTitle}>WELL DONE PARTNER RIDER!</Text>
          
          {/* Earnings Display */}
          <View style={styles.earningsContainer}>
            <Text style={styles.earningsLabel}>Earnings</Text>
            <Text style={styles.earningsAmount}>
              +{orderDetails?.rider_fee || '0.00'}
            </Text>
          </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.mapContainer}>
        <MapView
          ...
          onMapReady={() => {
            // On first ready, re-center using the rider top offset if we have their coords
            if (!didInitialPlacementRef.current && riderLocation) {
              didInitialPlacementRef.current = true;
              const initial = computeInitialRiderTopRegion(riderLocation, region);
              setRegionManually(initial);
              console.log('🧭 Initial map ready: applying rider top offset region');
            }
          }}
          onRegionChangeComplete={(r) => {
            // If user panned/zoomed, set suppression to avoid auto-fit snapping back
            suppressAutoFitUntilRef.current = Date.now() + SUPPRESS_AUTOFIT_MS;
          }}
          onPanDrag={() => {
            suppressAutoFitUntilRef.current = Date.now() + SUPPRESS_AUTOFIT_MS;
        ...
        >
          ...
          {/* Customer Location - Custom Asset Marker (shown when order is picked up/arrived) */}
          {customerLocation && (orderDetails?.status === 'otw' || orderDetails?.status === 'arrived') && (
            <Marker
        ...
        <View style={styles.floatingContainer}>
          ...
          <View style={styles.destinationTextContainer}>
            ...
            <Text style={styles.destinationAddress}>
              {formatFloatingDestinationAddress(orderDetails)}
            </Text>
          </View>
          ...
          {isRealTimeTracking && (
            <View style={styles.trackingIndicator}>
              <Animated.View style={[styles.trackingDot, { opacity: trackingDotOpacity }]} />
        ...
        <View style={styles.mapControls}>
          <TouchableOpacity 
            ...
            onPress={() => {
              ...
              if (riderLocation) {
                setRegionManually({
                  latitude: riderLocation.latitude + (region.latitudeDelta * 0.5 * -RIDER_TOP_OFFSET_FACTOR), // Keep rider at top
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              ...
              if (riderLocation) {
                setRegionManually({
                  latitude: riderLocation.latitude + (region.latitudeDelta * 2 * -RIDER_TOP_OFFSET_FACTOR), // Keep rider at top
                  longitude: riderLocation.longitude,
...
const styles = StyleSheet.create({
  ...
  // Note: trackingDot style is for static fallback. For animation, use Animated.View with this style.
  trackingDot: {
    width: 6,
    height: 6,
    borderRadius: 3,
