...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  const onHandlerStateChange = (event) => {
    if (event.nativeEvent.state === 5) { // END state
      ...
      const shouldExpand = translationY < -DRAG_LIMIT / 2 || velocityY < -500;
      const shouldCollapse = translationY > DRAG_LIMIT / 2 || velocityY > 500;
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Content Container - Overlay Layer (Outside Map Container) */}
