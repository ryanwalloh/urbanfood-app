...
const OrderTracking = ({ orderId, onBack }) => {
  ...
  const [region, setRegion] = useState({
    latitude: 8.2275 + (0.005 * 0.1), // Iligan City coordinates with 10% upward offset
    longitude: 124.2458,
  ...
  // Function to adjust map region to fit the entire route
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      // Center map on rider location with 10% upward offset for better positioning
      setRegion({
        latitude: latitude + (0.005 * 0.1), // Move center 10% of latitudeDelta upward
        longitude,
        latitudeDelta: 0.005, // Tighter zoom for better route visibility
        longitudeDelta: 0.005,
      });
