...
const RiderMainPage = ({ onLogout }) => {
  ...
  const connectToOrderUpdates = () => {
    try {
      ...
      const eventSource = apiService.connectToOrderUpdates(
        (data) => {
          // Handle incoming order count updates
          if (data.count !== undefined) {
            setOrdersCount(data.count);
            console.log('📦 Real-time order count update:', data.count);
          }
        },
        (error) => {
          console.log('❌ Real-time updates error:', error);
      ...
      console.log('❌ Failed to setup real-time updates:', error);
    }
  };
