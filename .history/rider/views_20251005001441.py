...
def get_presigned_upload_url(request):
    if request.method == 'POST':
        ...
        # AWS Config from settings (populated via .env)
        aws_access_key = (getattr(settings, 'AWS_ACCESS_KEY_ID', '') or '').strip()
        aws_secret_key = (getattr(settings, 'AWS_SECRET_ACCESS_KEY', '') or '').strip()
        aws_bucket = (getattr(settings, 'AWS_STORAGE_BUCKET_NAME', '') or '').strip()
        aws_region = (getattr(settings, 'AWS_S3_REGION_NAME', 'ap-southeast-2') or 'ap-southeast-2').strip()
        ...
        try:
            ...
            try:
                ...
                masked = f"{aws_access_key[:4]}...{aws_access_key[-4:]}" if aws_access_key else 'EMPTY'
                logging.getLogger(__name__).error(f"STS validation failed for key {masked}: {cred_err}")
                return JsonResponse({'success': False, 'message': f'AWS credentials invalid: {str(cred_err)}'}, status=500)

            # Params for PUT (no ACL header; compatible with bucket owner enforced)
            params = {
                'Bucket': aws_bucket,
                'Key': key,
