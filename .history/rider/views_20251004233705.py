...
def get_presigned_upload_url(request):
    if request.method == 'POST':
        ...
        # AWS Config from env (fallback to provided values)
        aws_access_key = os.environ.get('AWS_ACCESS_KEY_ID', 'AKIA5RA7TEOC2ZAU5MG2')
        aws_secret_key = os.environ.get('AWS_SECRET_ACCESS_KEY', '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')
        aws_bucket = os.environ.get('AWS_STORAGE_BUCKET_NAME', 'pharmago-user-uploads')
