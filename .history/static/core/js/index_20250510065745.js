...
document.getElementById('registerAccount').addEventListener('submit', function(e) {
    ...
    fetch('/registerAccount/', {
        ...
        body: JSON.stringify({ email: email, password: password, firstName: firstName, lastName: lastName, role: role })
    })
    .then(response => response.json())
    .then(data => {
        if (data.message) {
            messageElement.innerHTML = 'Successful Registration<br>Click the link we seent you to securely log in.';
        } else {
            document.getElementById('passwordMessage').innerText = data.error || 'Registration failed';
        }
    })
    .catch(error => {
        console.error('Error:', error);
