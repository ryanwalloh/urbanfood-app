...
def send_verification_code(request):
    ...
    try:
        ...
        role = data.get('role')  # optional hint from client: 'customer' | 'rider'
        ...
        if not success:
            ...
            if getattr(settings, 'DEBUG', False):
                logger.warning(f"Email/SMS send failed (DEBUG). Using fallback. To: {phone_number}, Code: {verification.code}, Error: {error_detail}")
                return JsonResponse({
                    'success': True,
...
def verify_sms_code(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            role = user_data.get('role') or 'rider'
