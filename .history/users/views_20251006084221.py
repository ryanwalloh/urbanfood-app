...
def send_verification_code(request):
    ...
    try:
        ...
        if sms_service.is_configured():
            ...
            if not success:
                ...
                if getattr(settings, 'DEBUG', False):
                    logger.warning(f"Twilio send failed (DEBUG). Using fallback. To: {phone_number}, Code: {verification.code}, Error: {error_detail}")
                    return JsonResponse({
                        'success': True,
        ...
        # No user data stored yet - will be sent during verification
