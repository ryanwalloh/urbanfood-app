...
def get_order_status(request, order_id):
    ...
    try:
        ...
        # Use try_get to safely handle cases where assigned_rider might be None
        try:
            order = Order.objects.select_related('assigned_rider').get(id=order_id)
        ...
        if hasattr(order, 'assigned_rider') and order.assigned_rider:
            order_data['assigned_rider_id'] = order.assigned_rider.id
            order_data['assigned_rider_name'] = f"{order.assigned_rider.first_name} {order.assigned_rider.last_name}"
            print(f'✅ Order {order.id} has assigned rider: {order.assigned_rider.id}')
        else:
            print(f'ℹ️ Order {order.id} has no assigned rider yet')
