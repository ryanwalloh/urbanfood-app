    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                # Try to parse Plus Code if it looks like one
                if '+' in address.street and len(address.street) > 8:
                    # Simple Plus Code decoder for Manila area
                    # This is a basic implementation - in production, use openlocationcode library
                    try:
                        # Extract the Plus Code part (before the +)
                        plus_code = address.street.split('+')[0]
                        if len(plus_code) >= 8:
                            # Basic decoding for Manila area (approximate)
                            # This is a simplified version - real Plus Code decoding is more complex
                            latitude = 14.5995 + (hash(plus_code[:4]) % 1000 - 500) / 100000
                            longitude = 120.9842 + (hash(plus_code[4:8]) % 1000 - 500) / 100000
