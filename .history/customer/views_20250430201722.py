...
def order_complete(request, order_id):
    try:
        order = Order.objects.select_related('restaurant', 'customer').get(id=order_id, customer=request.user.customer)
        order_lines = OrderLine.objects.filter(order=order).select_related('product')

        # Get the address (optional: you can use order.customer.address if you have a direct FK)
        address = Address.objects.filter(user=request.user).first()
